...
for i=1:N-2
    % Predictor meuler orden 1
    x(:,i+3)=x(:,i+2)+h*f(t(i+2),x(:,i+2));
    % orden precision P(EC)^mE min {p,p*+m} necesitamos m = p*-p = orden predictor menos orden corrector=4-1=3
    for j=1:3
        % corrector adam moulton 3 pasos orden 4
